...
const menuUserItemReducer = (state = initialState, action) => {
   switch (action.type) {
      ...
      case EROOR_TO_SUBMIT:
         ...
         if (state.data[action.payload.idx].prices.length < 2)
            state.data[action.payload.idx].errorMessage.prices = '가격을 작성해주세요.';
         if (state.data[action.payload.idx].menuAbout.length < 2)
            state.data[action.payload.idx].errorMessage.menuAbout = '메뉴 설명을 작성해주세요.';
         return state;
      default:
         return state;
   }
